...
def invoke(message):
    try:
        task = json.load(message[-1].content)
    ...
    for attempt in range(retry_limit):
        ...
        if rst['code']==0:
            ...
            break
        corrections = correct_sql(eva_sql, rst['message'], history, db, model_name)
...
def perform_correction(message,db,model_name):
    ...
    while True:
        ...
        rsp =tool_chain(model_name,message,[getable])
        message.append(rsp)
        result.append(rsp)
        for tool_call in rsp.tool_calls:
            selected_tool = getable if tool_call["name"].lower() == "getable" else None
            if selected_tool:
